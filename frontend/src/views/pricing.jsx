...
const PricingPage = () => {
    ...
    const handleDowngradeToFree = () => {
        ...
        if (storedUser) {
            ...
            userObj.plan_id = 1; // ปรับกลับเป็น 1
            userObj.plan_name = "Free Tier";
            localStorage.setItem('user', JSON.stringify(userObj));
            window.location.href = '/dashboard'; // เด้งกลับไปหน้า Dashboard
        } else {
                ...
                <div className="max-w-full mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
                    ...
                    <div className={`bg-white rounded-2xl border border-slate-200 p-8 flex flex-col shadow-sm ${cardHoverStyle}`}>
                        ...
                        <button
                            ...
                            className="w-full py-2 px-4 bg-[#0B152A] hover:bg-slate-800 text-white rounded-lg font-medium text-sm transition-colors mt-auto text-center cursor-pointer"
                        >
                            Start for free (Downgrade)
                        </button>
                    </div>
                    ...
                    <div className={`bg-white rounded-2xl border-2 border-yellow-400 p-8 flex flex-col relative shadow-xl transform md:-translate-y-2 ${cardHoverStyle} hover:-translate-y-5`}>
                        ...
                        <Link href="/checkout?plan=plus" className="w-full py-2 px-4 bg-gradient-to-br from-[#F6C65B] to-[#D99A1F] hover:brightness-90 text-black text-sm rounded-lg font-medium transition-colors mt-auto text-center">
                            Start with Plus
                        </Link>
                    </div>
