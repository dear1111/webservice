...
const chartData = [
  ...
  { time: "02:00", actual: 2373.1, projected: 2395.5 },
  { time: "04:00", actual: 2396.3, projected: 2402.5 },
  { time: "06:00", actual: 2408.9, projected: 2425.25 },
  { time: "08:00", actual: 2384.2, projected: 2390.1 },
  { time: "10:00", actual: 2369.9, projected: 2375.1 },
  { time: "12:00", actual: 2358.9, projected: 2362.1 },
  { time: "14:00", actual: 2362.16, projected: 2370.6 },
  { time: "16:00", actual: 2373.1, projected: 2377.2 },
  { time: "18:00", actual: 2391.52, projected: 2408.23 },
...
const Dashboard = () => {
  ...
  const getPlanDetails = (planId) => {
    ...
    if (id === 3)
      return { name: "Pro Tier 🏆", badge: "bg-purple-100 text-purple-700" };
    if (id === 2)
      return { name: "Plus Tier ⚡", badge: "bg-[#F6C65B]/20 text-[#D99A1F]" };
  ...
  useEffect(() => {
    ...
    if (!savedUser) {
      ...
      setUser(parsedUser); // 🌟 เพิ่มบรรทัดนี้ เพื่อให้โค้ดรู้จัก user
    ...
    const fetchRealGoldPrice = async () => {
      try {
        ...
        const response = await fetch(
          "https://api.coingecko.com/api/v3/coins/pax-gold/market_chart?vs_currency=thb&days=1",
        );
        ...
        const formattedData = data.prices.map((item) => {
          ...
          return {
            ...
            projected: parseFloat(
              (actualPrice * (1 + (Math.random() * 0.002 - 0.001))).toFixed(2),
            ),
          };
        });
        ...
        const simplifiedData = formattedData
  ...
  useEffect(() => {
    ...
    if (
      tabFromUrl &&
      ["Overview", "API Keys", "Usage Metrics"].includes(tabFromUrl)
    ) {
  ...
  const generateNewKey = async () => {
    try {
      ...
      const apiUrl =
        ...
        "https://shiny-space-winner-vprp94qgjxvcpv4r-5000.app.github.dev/api";
  ...
  const handleCancelPlan = () => {
    ...
    if (
      window.confirm(
        "Are you sure you want to cancel your current plan and return to Free Tier?",
      )
    ) {
  ...
  // กำหนด Limit ตามชื่อ Plan (ปรับชื่อ 'pro', 'plus' ให้ตรงกับใน Database ของคุณนะครับ)
// 🌟 เปลี่ยนมาเช็คจากเลข plan_id แทนข้อความ
  const getLimitByPlanId = (planId) => {
    const id = Number(planId) || 1;
    if (id === 3) return 100000; // Pro Tier
    if (id === 2) return 10000;  // Plus Tier
    return 1000;                 // Free Tier
  };

  // ดึงค่า Limit จาก plan_id ของ user แทน
  const maxLimit = getLimitByPlanId(user?.plan_id);
            ...
            <nav className="space-y-2">
              {["Overview", "API Keys", "Usage Metrics"].map((item) => (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all ${activeTab === item ? "bg-gradient-to-br from-[#F6C65B] to-[#D99A1F] text-black shadow-lg" : "text-slate-400 hover:text-white hover:bg-white/5"}`}
                >
                  {item === "Overview" && (
            ...
            <>
              <header>
                ...
                <p className="text-slate-500">
                  Welcome back, check your service health below.
                </p>
                {/* ป้ายบอกสถานะแพ็กเกจ (Plan Badge) */}
                <div
                  className={`flex items-center justify-between px-5 py-3 rounded-2xl shadow-sm ${userPlan.badge}`}
                >
              ...
              {/* ปิด div ของ Grid ตรงนี้สำคัญมาก */}
              ...
              <div className="bg-white p-8 rounded-3xl border border-slate-100 h-[400px] shadow-sm">
                <div className="mb-6">
                  ...
                  <p className="text-xs text-slate-400">
                    Real-time market chart (THB / Baht weight)
                  </p>
                </div>
            ...
            <div className="bg-white rounded-3xl border border-slate-100 p-8 shadow-sm">
              <div className="flex justify-between items-center mb-8">
                ...
                <button
                  ...
                  className="bg-[#0B152A] text-white px-6 py-3 rounded-2xl text-sm font-bold flex items-center gap-2 hover:bg-black transition-all cursor-pointer"
                >
                    ...
                    <tr key={item.id}>
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-2">
                          <div className="w-24 h-2 bg-slate-100 rounded-full overflow-hidden">
                            {/* ปรับให้ขยับตาม usage จริงและ maxLimit ของแพ็กเกจ */}
                            <div
                              className="h-full bg-yellow-500 transition-all duration-500"
                              style={{
                                width: `${Math.min((item.usage / maxLimit) * 100, 100)}%`,
                              }}
                            ></div>
                          </div>
                          {/* แสดงตัวเลขแบบไดนามิก */}
                      ...
                      <td className="p-4 text-right">
                        <button
                          ...
                          className={`flex items-center justify-end gap-2 px-3 py-1.5 rounded-lg font-bold text-xs transition-all duration-200 ${
                            copiedId === item.id
          ...
          {activeTab === "Usage Metrics" && (
            <div className="bg-white rounded-3xl border border-slate-100 p-8 shadow-sm text-center py-20">
              ...
              <p className="text-slate-500 max-w-sm mx-auto mt-2">
                You have used <b>{totalUsage.toLocaleString()}</b> requests out
                of your {maxLimit.toLocaleString()} daily limit.
              </p>
              ...
              <div className="max-w-md mx-auto mt-8">
                ...
                <div className="flex justify-between mt-2 text-xs font-bold text-slate-400">
                  <span>0%</span>
                  <span>
                    {Math.min((totalUsage / maxLimit) * 100, 100).toFixed(1)}%
                    used
                  </span>
                  <span>100%</span>
                </div>
