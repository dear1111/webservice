...
const rankingData = [
  { id: 1, name: "Hua Seng Heng", spread: 8, making: 250, trend: "down" },
  { id: 2, name: "YLG Bullion", spread: 10, making: 280, trend: "down" },
  { id: 3, name: "MTS Gold", spread: 12, making: 300, trend: "up" },
  { id: 4, name: "Ausiris", spread: 14, making: 320, trend: "down" },
  { id: 5, name: "Classic Gold", spread: 16, making: 350, trend: "up" },
];
...
const SentimentGauge = () => (
  <div className="relative w-full max-w-[180px] mx-auto mt-2">
    <svg viewBox="0 0 100 60" className="w-full overflow-visible">
      ...
      <g style={{ transformOrigin: "50px 50px", transform: "rotate(40deg)" }}>
        <line x1="50" y1="50" x2="50" y2="18" stroke="#0B152A" strokeWidth="2" strokeLinecap="round" />
    ...
    <div className="flex-1 justify-center text-center pt-2">
      <p className="text-3xl font-bold text-[#0B152A]">
        72<span className="text-sm text-slate-400 font-medium">/100</span>
      </p>
      <p className="text-xs font-bold text-green-500 mt-1 uppercase tracking-widest">Bullish</p>
    </div>
  </div>
);
    ...
    <div className="flex flex-col">
      ...
      <section className="bg-linear-to-br from-[#050F1F] to-[#0A1D3A] text-white pt-26 pb-16 px-4">
        ...
        <div className="max-w-full mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center z-10 relative">
          <div className="space-y-5 py-4">
            <p className="inline-flex items-center gap-2 px-4 py-0.5 rounded-lg bg-[rgba(230,184,76,0.1)] border border-[rgba(230,184,76,0.6)] text-[#F2C94C] text-xs">
              Live · LBMA-aligned
            </p>
            ...
            <div className="flex space-x-4">
              <Link href="/dashboard" className="bg-gradient-to-br from-[#F6C65B] to-[#D99A1F] hover:bg-yellow-500 shadow-md shadow-[#D99A1F]/50 text-gray-800 px-7 py-2.5 rounded-md font-semibold text-[13px] inline-block text-center">
                Start free →
              </Link>
              <Link href="/docs" className="hover:bg-slate-800 border border-slate-400 text-white px-7 py-2.5 rounded-md font-semibold text-[13px] inline-block text-center">
                View API docs
              </Link>
            </div>
            <div className="max-w-full mx-auto border-t border-gray-700 py-10 mt-10">
              <div className="flex space-between">
                <div className="flex-1">
                  <p className="text-2xl bg-gradient-to-br from-[#F6C65B] to-[#D99A1F] bg-clip-text text-transparent font-bold">&lt; 1s</p>
                  <p className="text-[12px] text-gray-400">UPDATE FREQ</p>
                </div>
                <div className="flex-1">
                  <p className="text-2xl bg-gradient-to-br from-[#F6C65B] to-[#D99A1F] bg-clip-text text-transparent font-bold">120+</p>
                  <p className="text-[12px] text-gray-400">SHOPS INDEXED</p>
                </div>
                <div className="flex-1">
                  <p className="text-2xl bg-gradient-to-br from-[#F6C65B] to-[#D99A1F] bg-clip-text text-transparent font-bold">99.99%</p>
          ...
          <div className="bg-[#f8fafc] rounded-2xl p-6 md:p-8 shadow-2xl w-full max-w-3xl mx-auto font-sans relative z-10 border border-slate-100">
            <div className="flex justify-between items-start mb-6">
              <div>
                <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">XAU / THB</p>
                <h3 className="text-4xl font-bold text-[#0B152A]">฿2,398.40</h3>
              </div>
              <div className="bg-green-100 text-green-600 text-xs font-bold px-3 py-1.5 rounded-full">+0.82%</div>
            </div>
            <div className="h-[250px] w-full mt-4">
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={chartData} margin={{ top: 10, right: 0, left: -20, bottom: 0 }}>
                  ...
                  <XAxis dataKey="time" axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: "#94A3B8" }} dy={10} ticks={["00:00", "04:00", "08:00", "12:00", "16:00", "20:00"]} />
                  <YAxis domain={["dataMin - 10", "dataMax + 10"]} axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: "#94A3B8" }} tickCount={5} />
                  <Tooltip content={<CustomTooltip />} cursor={{ stroke: "#cbd5e1", strokeWidth: 1, strokeDasharray: "4 4" }} />
                  <Line type="monotone" dataKey="projected" stroke="#64748B" strokeWidth={2} strokeDasharray="4 4" dot={false} activeDot={{ r: 4, fill: "#64748B", stroke: "#fff", strokeWidth: 2 }} />
                  <Area type="monotone" dataKey="actual" stroke="#EAB308" strokeWidth={2.5} fillOpacity={1} fill="url(#colorActual)" activeDot={{ r: 5, fill: "#EAB308", stroke: "#fff", strokeWidth: 2 }} />
                </ComposedChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            <div className="shadow-2xl rounded-xl">
              <div className="bg-[#0A1D3A] rounded-t-xl p-4 flex items-center justify-between border-b border-slate-700">
                ...
                <div className="text-slate-400 text-xs font-mono">embed.html</div>
              ...
              <div className="bg-[#0A1D3A] p-6 text-sm font-mono overflow-x-auto text-slate-300">
                <pre>
                  ...
                  <span className="text-slate-300">src=&quot;https://cdn.aurumindex.io/widget.js&quot;</span><br />
                  <span className="text-slate-300 pl-8">data-key=&quot;public&quot; data-symbol=&quot;XAUTHB&quot;</span><br />
                  <span className="text-slate-300 pl-8">data-theme=&quot;light&quot;</span>
                  <code className="text-blue-300">{`></script>`}</code>
                </pre>
              </div>
              <div className="bg-white rounded-b-xl p-6 border-x border-b border-slate-200">
                <div className="flex justify-between items-start mb-2">
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Gold Spot · 15M Delayed</p>
                ...
                <div className="flex justify-between items-end">
                  ...
                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Powered by Aurumindex</p>
                </div>
              </div>
            </div>
          </div>
              ...
              <table className="w-full text-left text-sm text-slate-700">
                <thead className="text-slate-500 text-xs border-b border-slate-200">
                  <tr>
                    ...
                    <th className="py-4 px-4 font-medium text-right">Spread (THB)</th>
                    <th className="py-4 px-4 font-medium text-right">Making (THB)</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {rankingData.map((shop, index) => (
                    <tr key={shop.id} className="hover:bg-slate-50 transition-colors">
                      <td className={`py-4 px-4 ${index === 0 ? '' : 'text-slate-400 pl-6'}`}>
                        {index === 0 ? (
                          <span className="flex items-center justify-center w-6 h-6 border border-yellow-400 text-yellow-600 rounded-full text-xs font-bold">1</span>
                        ) : (
                          shop.id
                        )}
                      </td>
                      <td className="py-4 px-4 font-medium text-[#0B152A]">{shop.name}</td>
                      <td className="py-4 px-4 text-right">{shop.spread}</td>
                      <td className="py-4 px-4 text-right">{shop.making}</td>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 h-full">
              ...
              <div className="flex flex-col space-y-4">
                <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex-1 flex flex-col justify-center">
                  ...
                  <h4 className="font-bold text-[#0B152A] text-sm mb-1">Real-time WebSocket</h4>
                  <p className="text-xs text-slate-500">&lt; 200 ms tick latency</p>
                </div>
                <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex-1 flex flex-col justify-center">
                  <ArrowPathIcon className="w-5 h-5 text-yellow-500 shrink-0" />
                  <h4 className="font-bold text-[#0B152A] text-sm mb-1">Webhooks</h4>
                  <p className="text-xs text-slate-500">Price • spread • volatility events</p>
                </div>
                <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex-1 flex flex-col justify-center">
                  <ShieldCheckIcon className="w-5 h-5 text-yellow-500 shrink-0" />
                  <h4 className="font-bold text-[#0B152A] text-sm mb-1">99.99% SLA</h4>
                  <p className="text-xs text-slate-500">Priority support included</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="bg-linear-to-br from-[#07162D] to-[#0D2445] py-24 flex justify-center items-center text-white">
        <div className="max-w-3xl text-center">
          <h2 className="text-4xl font-bold mb-2">Build the next generation of gold-market apps</h2>
          <p className="text-sm mb-6 text-gray-400">Sign up in 60 seconds. Free tier, no credit card required.</p>
          <Link href="/dashboard" className="inline-flex items-center justify-center bg-gradient-to-br from-[#F6C65B] to-[#D99A1F] shadow-lg shadow-[#D99A1F]/25 text-slate-800 px-8 py-3 rounded-md font-semibold text-sm hover:bg-yellow-500 transition-colors">
